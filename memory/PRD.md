...
## Auth hardening + Save Video (session 2 cont., iter22 verified 12/12)
- EMERGENT GOOGLE AUTH per playbook: api.ts token storage -> expo-secure-store on NATIVE (transparent one-time migration from AsyncStorage, so nobody logged out), web keeps AsyncStorage/localStorage. index.tsx: WebBrowser.maybeCompleteAuthSession() at module scope; onSignIn native registers Linking url listener BEFORE openAuthSessionAsync, falls back result.url -> listener -> getInitialURL (Android returns dismiss+no-url on success); usedSessionIds Set guards duplicate exchange; extractSessionId([?#&]) raw regex; cold-start getInitialURL session_id handling in mount effect. auth-callback.tsx: reads hash+search, cleans URL only AFTER successful exchange. Backend startup: user_sessions indexes (session_token unique, user_id, expires_at TTL=0), users.user_id unique. expo-secure-store + expo-sharing installed.
- LOGOUT: fire-and-forget /auth/logout (was awaiting -> could hang on flaky mobile networks = user's recurring "logout doesn't work"). Verified: session revoked server-side (401 after), instant redirect to login.
- SAVE VIDEO: analysis/[id].tsx saveVideo() + download-outline button (testID save-video-btn) in video controls, all tiers: web <a download> same-origin; native FileSystem.downloadAsync -> Sharing.shareAsync (share sheet has "Save Video"). 
- UPLOAD GUARD: upload.tsx rejects clips >200MB pre-upload with friendly message (record 1080p not 4K); tip5 added.
